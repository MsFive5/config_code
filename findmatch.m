%% 
function [trainingsection,distance,flag,signature] = findmatch(input,templatestructure,threshold)
flag = 0;
trainingsection = [];
signature = [];
...
for n = 1:40:inputlen - templatelen
    ...
    if ((std(currentdata(:,1)) + std(currentdata(:,2)) + std(currentdata(:,3)))<0.1) ||((std(currentdata(:,1)) + std(currentdata(:,2)) + std(currentdata(:,3)))>0.4)
       continue;
   else
...
for n = 1:countsignature
    for m = trainingsection(n).start:40:inputlen-400
        if (std(input(m:m+400,1)) + std(input(m:m+400,2)) + std(input(m:m+400,3)))>0.05
            continue;
        else
            endflag = 1;
