...
trainingpath = '~/Dropbox/01_research/08_BiClassifier/data/117/training/2015-02-06';
datapath = '~/Dropbox/01_research/08_BiClassifier/data/117';
...
%% read merge.csv and convert to dailyprocessMerge.mat
% dataLcell = {};
% dataRcell = {};
% Lcount = 0; 
% Rcount = 0;
...
data = data(1:1e5,:); % for debugging purposes

input.data = [data(:,1),data(:,2:4)/2048, data(:,12:14)/2048];
input.header = header;
...
[dtwLabel,walksectiondtw] = combinelabel2(labelleft,labelright);

% use NB to characterize walksection
trainingstructure = load([trainingpath '/' 'NaiveBayes_training.mat']);
nbLabel = DailyProcess(output,trainingstructure,trainingpath,trainingsensor_info.sensor_info, str2num(configStruc{2}.value{1}));

% combine the result from NB and DTW
walklabel = generatewalklabel(output,dtwLabel,walksectiondtw,nbLabel,datapath);
[walksection] = combineNBandDTW2(output,dtwLabel,walksectiondtw,nbLabel,datapath,walklabel);
mkdir([datapath '/results/'])
save([datapath '/results/' 'walklabel.mat'],'walklabel');


%% generating the daily summary out of the classification
