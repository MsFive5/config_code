function countfile = writefile(path,dirname,filestring,indexstart,indexend,countfile,time,para)
    ...
    if (timeend-timestart)<=59*normalcoeff
        ...
        fprintf(fid,';max speed is %f, average speed is %f, slowest speed is %f,average ratio is %f\n',max(para(indexstart:indexend,2)),mean(para(indexstart:indexend,2)),min(para(indexstart:indexend,2)),mean(para(indexstart:indexend,4)));
        for m = indexstart:indexend
            fprintf(fid,'%s\t%f %f %f %f \n',time(m).string,para(m,1),para(m,2),para(m,3),para(m,4));
        end
        fclose(fid);
