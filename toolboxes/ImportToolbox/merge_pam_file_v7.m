...
%
function [outputheader,outputdata,err_msg]= merge_pam_file_v7(sensor_info)
err_msg = [];
outputheader=[];
outputdata = [];
...
for k=1:num_sensors
    ...
    for p=1:num_of_files
        ...
        [mdata,header]=read_pam_csv_data_v6(file_name,0);
        
if(1)
        header.old_start_time = header.start_time;
        tmpVal = header.start_time(3);
        header.start_time(3) = 0;
        mdata(:,1) = mdata(:,1) + tmpVal;
else
        header.old_start_time = header.start_time;
        header.old_start_time(3) = 0;
end
...
for k=1:num_sensors
    ...
    for p=1:num_of_files
        ...
        if(1)
            ...
            tmp = diff(mdata(:,1));
            idx = find(tmp<0);
            tmp_offset = data{k}{p}.header.old_start_time(3);
...
%                 fprintf('%.3f -> %.3f, ', [mdata(idx,1)'; mdata(idx+1,1)']-tmp_offset);
%                 fprintf('\n\n');
            end
...
if(0)        
        ...
        %round off the interval of the rows to the nearest sample_interval.
        new_time = round(mdata(:,1)*sample_rate);
        max_step = round((max(new_time)-min(new_time)))+1;
        
        %create a full matrix for the samples in the data.
        row = max_step;
%         new_data = zeros(row+1, 4);
%         new_data(:,1) =  new_time(1,1) + [0:row]';
        if length(row) == 0
            continue;
        else
            new_data = zeros(row+1, 4);    
...
% 1. within a sec, move the points to the closet right position
% 2. pad if necessary
if(1)
    new_time = floor(mdata(:,1));
    new_time_seed = unique(new_time,'first');
    testarray = [];
    totallen = 0;
    new_shifttime = [];
    for n = 1:length(new_time_seed)
        currentseedind = find(new_time == new_time_seed(n));
        totallen = totallen+length(currentseedind);
        timeoffset = mdata(currentseedind(1),1)-new_time_seed(n);
        if abs(length(currentseedind)-sample_rate+round(timeoffset*sample_rate))<=3 
            new_shifttime(currentseedind) = (currentseedind-currentseedind(1)+1)/sample_rate+new_time_seed(n);       
        end        
    end
end

        %round off the interval of the rows to the nearest sample_interval.
        new_time = round(new_shifttime*sample_rate);
        max_step = round((max(new_time)-min(new_time)))+1;
...
% new_data = zeros(length(new_shifttime),4);
% new_data(:,1) = new_shifttime;
% new_data(:,2:4) = mdata(1:length(new_shifttime),2:4);
% diffval = diff(new_shifttime);
% idx = find(diffval>(1/sample_rate*2-1e-6));
% 
% while(length(idx)>=1)
%     length(idx);
%     padlen = floor((new_data(idx(1)+1,1)-new_data(idx(1)))*sample_rate)-1;
%     
%     updated_time = [new_data(1:idx(1),1); ([1:padlen]/sample_rate+new_data(idx(1),1))';new_data(idx(1)+1:end,1)];
%     updated_data = zeros(length(updated_time),4);
%     updated_data(:,1) = updated_time;
%     updated_data(1:idx(1),:) = new_data(1:idx(1),:);    
%     %updated_data()    
%     if padlen>1
%         updated_data(idx(1)+1:idx(1)+padlen,2:4) = repmat(new_data(idx(1),2:4),padlen,1);
% %         new_data(idx(1)+1:idx(1)+padlen,2:4) = new_data(idx(1),2:4);  
%     else
%         updated_data(idx(1)+1,2:4) = new_data(idx(1),2:4); 
%     end
%     updated_data(idx(1)+padlen+1:end,2:4) = new_data(idx(1)+1:end,2:4);
% 
%     new_data = updated_data;
%     
%     diffval = diff(new_data(:,1));
%     idx = find(diffval>(1/sample_rate*2-1e-6));
% end
...
% %             fprintf('%.3f -> %.3f, ', [mdata(IB(idx),1)'; mdata(IB(idx)+1,1)']-tmp_offset);
% %             fprintf('\n\n'); 
%         end
